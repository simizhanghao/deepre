...
case "$mode" in
  ...
  segment)
    ...
    save_freq=200
    resume_mode=auto
    experiment=qwen3_30b_evidence_800
    ;;
  *)
...
log="$PROJECT_ROOT/logs/grpo_${mode}_to${target_step}_${stamp}.log"
...
echo "GRPO mode=$mode current=$current target=$target_step batch=$batch n=$n micro=$micro"
echo "log=$log"
