#!/usr/bin/env bash
# Evaluate one RF++-baseline staged checkpoint.
set -euo pipefail
...
model=$repo/results/20_rfpp_baseline/checkpoints/step${step}_hf
out=$repo/results/20_rfpp_baseline/eval/step${step}
capture=$out/route_capture
sentinel=$repo/outputs/rfpp_baseline_sentinel_step${step}
...
env -u LD_LIBRARY_PATH "$vexact_repo/.venv/bin/python" \
  "$repo/scripts/summarize_boundary_node.py" --profile rfpp_baseline \
  --step "$step" --metrics "$repo/results/20_rfpp_baseline/train_metrics.jsonl" \
...
echo "RFPP_BASELINE_NODE_${step}_EVAL_COMPLETE"
