...
# Prerequisites: HF merges under outputs/rl/hf_merged/ (see export_verl_fsdp_to_hf.sh)
set -euo pipefail
...
OUT_ROOT=${OUT_ROOT:-results/phase3c_gen_val200_${STAMP}}
...
run_one() {
  ...
  CUDA_VISIBLE_DEVICES=$gpu python scripts/run_agent_rollout_smoke.py \
    ...
    --output-dir "$OUT_ROOT" \
    --run-tag "$tag" \
    2>&1 | tee "logs/phase3c_gen_${tag}_${STAMP}.log"
}

SFT=${SFT:-outputs/sft_qwen25_3b_coldstart_v1_merged}
B3=${B3:-outputs/rl/hf_merged/grpo_sftv1_smoke_step100}
C3=${C3:-outputs/rl/hf_merged/grpo_sftv1_evidence_3c_step400}
