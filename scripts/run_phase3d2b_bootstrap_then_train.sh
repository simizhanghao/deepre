...
#
# Usage:
#   BOOTSTRAP_PID=125475 nohup bash scripts/run_phase3d2b_bootstrap_then_train.sh \
#     > logs/phase3d2b_chain.log 2>&1 &
set -euo pipefail
...
BOOTSTRAP_LOG=${BOOTSTRAP_LOG:-$REPO/logs/boundary_bootstrap_3c400_full.log}
BOUNDARY_LATEST=${BOUNDARY_LATEST:-$REPO/outputs/rl/boundary/boundary_latest.json}
...
"$PY" "$REPO/scripts/audit_boundary_table.py" \
  ...
  --out "$REPO/outputs/rl/boundary/boundary_audit_pre_chain.json"
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-grpo_3c400_boundary_3d2b}
export OUT_DIR=${OUT_DIR:-/workspace/deepresearch/outputs/rl/grpo_3c400_boundary_3d2b}
export MODEL_PATH=${MODEL_PATH:-/workspace/deepresearch/outputs/rl/hf_merged/grpo_sftv1_evidence_3c_step400}
export ECA_BOUNDARY_TABLE=${ECA_BOUNDARY_TABLE:-/workspace/deepresearch/outputs/rl/boundary/boundary_latest.json}
...
log "train log symlink: $REPO/logs/grpo_3d2b_latest.log"
