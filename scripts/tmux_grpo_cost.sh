#!/usr/bin/env bash
# Host-side Phase 3D1 Uniform Cost launcher (detach-safe). Fresh from SFT-v1.
# Usage:
...
SESSION=${SESSION:-eca-grpo-3d1}
CONTAINER=${CONTAINER:-eca-verl}
STEPS=${STEPS:-400}
SAVE_FREQ=${SAVE_FREQ:-50}
OUT_DIR=${OUT_DIR:-/workspace/deepresearch/outputs/rl/grpo_sftv1_cost_3d1}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-grpo_sftv1_cost_3d1}
RESUME_MODE=${RESUME_MODE:-disable}
ECA_EVIDENCE_WEIGHT=${ECA_EVIDENCE_WEIGHT:-0.5}
ECA_SEARCH_COST_WEIGHT=${ECA_SEARCH_COST_WEIGHT:-0.40}
TB_PORT=${TB_PORT:-6008}
HOST_TB_DIR=${HOST_TB_DIR:-$REPO/outputs/rl/tensorboard/$EXPERIMENT_NAME}
STAMP=$(date +%Y%m%d_%H%M%S)
LOG_NAME=${LOG_NAME:-grpo_${EXPERIMENT_NAME}_to${STEPS}_ls${ECA_SEARCH_COST_WEIGHT}_${STAMP}.log}
...
docker exec "$CONTAINER" bash -lc \
  "mkdir -p /workspace/deepresearch/outputs/rl/tensorboard/${EXPERIMENT_NAME} \
            /workspace/deepresearch/outputs/rl/grpo_sftv1_cost_3d1 \
...
: >"$LOG"
ln -sfn "$LOG" "$REPO/logs/grpo_3d1_latest.log"
...
sleep 3
if ! docker exec "$CONTAINER" bash -lc "pgrep -f launch_grpo_main.py >/dev/null || pgrep -f run_grpo_cost.sh >/dev/null"; then
...
echo "Started Phase 3D1 Uniform Cost (detach-safe): STEPS=$STEPS λ_s=$ECA_SEARCH_COST_WEIGHT"
echo "  attach:  tmux attach -t $SESSION"
echo "  log:     $LOG"
echo "  TB:      http://127.0.0.1:$TB_PORT"
echo "  ckpt:    $REPO/outputs/rl/grpo_sftv1_cost_3d1"
echo "  init:    SFT-v1 merged (NOT 3C@400)"
echo "  stop if: NaN / finish collapse / KL explode / search→0 with answer collapse"
