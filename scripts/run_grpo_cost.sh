#!/usr/bin/env bash
# Phase 3D1 Uniform Cost GRPO — fresh from SFT-v1 (NOT from 3C@400).
# R = EM + 0.5 EvidF1 + 0.1 Format − λ_s * N_search
# Default: λ_s=0.40 (3D0 operational), STEPS=400, SAVE_FREQ=50.
set -euo pipefail
...
export ECA_SEARCH_COST_WEIGHT=${ECA_SEARCH_COST_WEIGHT:-0.40}
...
OUT_DIR=${OUT_DIR:-$REPO/outputs/rl/grpo_sftv1_cost_3d1}
STEPS=${STEPS:-400}
...
echo "[3D1] fresh SFT-v1 STEPS=$STEPS λ_e=$ECA_EVIDENCE_WEIGHT λ_s=$ECA_SEARCH_COST_WEIGHT OUT=$OUT_DIR resume=$RESUME_MODE"
