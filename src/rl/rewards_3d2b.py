"""Phase 3D2b — Search-Boundary-Aware ECA reward.
...
Rewards (ECA adaptation of SAAS boundary + Evidence):

  NoSearch:      R = R_A + 0.1 R_F − α 1[N_s>0]     (Evidence OFF)
  NeedSearch:    R = R_A + λ_e R_E + 0.1 R_F         (no search cost; v1)
...
def get_boundary_table() -> Dict[str, str]:
    ...
    if not _LOGGED_HASH:
        print(
            f"[3D2b] loaded ECA_BOUNDARY_TABLE path={_BOUNDARY_META.get('path')} "
...
def compute_score(
    ...
    **kwargs,
    ...
    if boundary == "NoSearch":
        eff_e = 0.0
        eff_s = float(base_w.search_cost_weight)  # α · 1[search]
    else:
        # NeedSearch / Undetermined: full Evidence, no search cost (v1)
